...
def load_models(base_model_path, sft_model_path):
    ...
    """
    model_a = AutoModelForCausalLM.from_pretrained(base_model_path, device_map="cpu")
    model_b = AutoModelForCausalLM.from_pretrained(sft_model_path, device_map="cpu", local_files_only=True)
...
def analyze_layerwise_similarity(model_a, model_b, pattern):
    ...
    for param_name in shared_params:
        ...
        if match:
            ...
            rel_dis = torch.dist(params_a[param_name], params_b[param_name]) / torch.norm(params_a[param_name])
            rel_dis = rel_dis.detach().cpu().numpy()
            layerwise_similarities[layer_idx].append((param_name, sim))
            layerwise_relative_dis[layer_idx].append((param_name, rel_dis))
...
def plot_layerwise_average_change(
    ...
    attn_all_changes = [sim for layer in self_attn_changes for _, sim in self_attn_changes[layer]]
    # 计算平均值
    attn_average_sim = sum(attn_all_changes) / len(attn_all_changes)

    mlp_all_sims = [sim for layer in mlp_changes for _, sim in mlp_changes[layer]]
    # 计算平均值
    mlp_average_sim = sum(mlp_all_sims) / len(mlp_all_sims)
    
    content = ''
    
    param_name_to_changes = defaultdict(list)
    
    for layer in layers:
        content += '################ {} ################\n'.format(layer)
        
        line = ''
        for idx in range(len(self_attn_changes[layer])):
            param_name, changes = self_attn_changes[layer][idx]
            param_name = param_name.split('.')[3] + '.' + param_name.split('.')[4]
            param_name_to_changes[param_name].append(changes)
            line += f"{param_name} {name}={changes:.4f}\n"

        for idx in range(len(mlp_changes[layer])):
            param_name, changes = mlp_changes[layer][idx]
            param_name = param_name.split('.')[3] + '.' + param_name.split('.')[4]
            param_name_to_changes[param_name].append(changes)
            line += f"{param_name} {name}={changes:.4f}\n"
    ...
    for param_name, changes in param_name_to_changes.items():
        content += f"{param_name} {name}={sum(changes) / len(changes):.4f}\n"
    ...
    plt.hist([attn_all_changes, mlp_all_sims], bins, alpha=0.5, label=["self-attn", "mlp"])
...
if __name__ == "__main__":
    ...
    sft_model_paths = [
    #    f"skythought/saves/Open-Thoughts-2k-10k/Llama-3.1-8B-Instruct/full/checkpoint-936",
    #    f"skythought/saves/Open-Thoughts-2k-10k/Llama-3.1-8B-Instruct/lora-64/complete_ckpt",
    #    f"skythought/saves/Open-Thoughts-2k-10k/Llama-3.1-8B-Instruct/lora-128/complete_ckpt",
    #    f"skythought/saves/Open-Thoughts-16k-10k/Llama-3.1-8B-Instruct/full/checkpoint-936",
    #    f"skythought/saves/Open-Thoughts-16k-10k/Llama-3.1-8B-Instruct/lora-64/complete_ckpt",
    #    "skythought/saves/Open-Thoughts-2k-10k/Qwen2.5-7B-Instruct/lora-64_emb/complete_ckpt",
    #    "skythought/saves/Open-Thoughts-2k-10k/Qwen2.5-7B-Instruct/lora-128_emb/complete_ckpt",
    #    "skythought/saves/Open-Thoughts-16k-10k/Qwen2.5-7B-Instruct/lora-64_emb/complete_ckpt",
    
    #    "skythought/saves/Bespoke-Stratos-17k/Qwen2.5-7B-Instruct/lora-64/complete_ckpt",
