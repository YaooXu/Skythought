...
class Qwen2MLP(nn.Module):
    ...
    def __init__(
        ...
        if os.environ['SHIFT_VERSION'] in ['v3.5', 'v3.5-abl1', 'v3.5-abl2', 'v3.6']:
    ...
    def forward(self, x):
        ...
        if os.environ['SHIFT_VERSION'] == 'v3.6':
            alpha = F.sigmoid(self.scale(torch.concat([x_shift, x], dim=-1))[0])
            alpha1 = alpha[:, 0].unsqueeze(-1)
            alpha2 = alpha[:, 0].unsqueeze(-1)
            shift_gate = self.W(self.R(torch.concat([x_shift * alpha1, x * alpha2], dim=-1))[0])[0]
