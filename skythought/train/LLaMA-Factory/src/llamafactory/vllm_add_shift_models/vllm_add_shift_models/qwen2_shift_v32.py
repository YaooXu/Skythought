...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # v1
        # score = F.softmax(self.shift_weight(torch.cat([x, x_shift], dim=-1))[0], dim=-1).bfloat16()
        # conv_x = score[:, 0].unsqueeze(-1) * x + score[:, 1].unsqueeze(-1) * x_shift
        
        # # v2
        # score = self.shift_weight(x_shift)[0]
        # conv_x = (1 - score) * x + score * x_shift
        # final_gate = self.gate_proj(conv_x)[0]
        
        # v2.2
        # score = self.shift_weight(torch.cat([x, x_shift], dim=-1))[0]
        
        # score1 = score[:, 0].unsqueeze(-1)
        # conv_x1 = (1 - score1) * x + score1 * x_shift
        
        # score2 = score[:, 1].unsqueeze(-1)
        # conv_x2 = (1 - score2) * x + score2 * x_shift
        # gate, _ = self.gate_proj(conv_x1)
        # up, _ = self.up_proj(conv_x2)
        
        # # v2.3
        # rotated_base = self.R(x)[0]
        # conv_x = x + torch.matmul(
        # (self.W(x_shift)[0] - rotated_base), self.R.weight
        # )
        # gate, _ = self.gate_proj(conv_x)
        # up, _ = self.up_proj(conv_x)
        
        # x = self.act_fn(gate) * up
        # x, _ = self.down_proj(x)
        
        # v3.1
        alpha = F.sigmoid(self.scale(x_shift - x)[0]) * 2
        
        shift_gate = self.W(self.R(x_shift - x)[0])[0] * alpha
