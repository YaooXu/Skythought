...
class Qwen2MLP_shift(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        # self.shift_weight= nn.Linear(3584*2, 1, bias=False)

        # self.R = nn.Linear(3584, rank, bias=False)
        # self.W = nn.Linear(3584, rank, bias=True)
    ...
    def forward(self, hidden_state):
        ...
        if length > 1:
            ...
            # v1
            # score = F.softmax(self.shift_weight(torch.cat([hidden_state, hidden_state_shift], dim=-1)), dim=-1).bfloat16()
            # conv_hidden_state = score[:, :, 0].unsqueeze(2) * hidden_state + score[:, :, 1].unsqueeze(2) * hidden_state_shift

            # # v2 gate
            # score = self.shift_weight(hidden_state_shift)
            # conv_hidden_state = (1 - score) * hidden_state + score * hidden_state_shift
            # return self.down_proj(self.act_fn(conv_hidden_state) * self.up_proj(hidden_state))

            # # v2.1 gate and up
            # score = self.shift_weight(torch.cat([hidden_state, hidden_state_shift], dim=-1))
            # conv_hidden_state = (1 - score) * hidden_state + score * hidden_state_shift
            # return self.down_proj(self.act_fn(self.gate_proj(conv_hidden_state)) * self.up_proj(conv_hidden_state))

            # # v2.2 gate and up two score
            # score = self.shift_weight(torch.cat([hidden_state, hidden_state_shift], dim=-1))
            # score1 = score[:, :, 0].unsqueeze(2)
            # conv_hidden_state_1 = (1 - score1) * hidden_state + score1 * hidden_state_shift

            # score2 = score[:, :, 1].unsqueeze(2)
            # conv_hidden_state_2 = (1 - score2) * hidden_state + score2 * hidden_state_shift

            # return self.down_proj(self.act_fn(self.gate_proj(conv_hidden_state_1)) * self.up_proj(conv_hidden_state_2))

            # # v2.3 inter
            # # bsz, hidden_size, intermediate_size
            # rotated_base = self.R(hidden_state)
            # conv_hidden_state = hidden_state + torch.matmul((self.W(hidden_state_shift) - rotated_base), self.R.weight)
            
            # return self.down_proj(self.act_fn(self.gate_proj(conv_hidden_state)) * self.up_proj(conv_hidden_state))

            # # v3
            # score = self.shift_weight(torch.cat([hidden_state, hidden_state_shift], dim=-1))
            # shift_gate = (self.gate_b(self.gate_a(hidden_state_shift)) + self.gate_proj(hidden_state_shift)) * score

            # ori_gate = self.gate_proj(hidden_state)

            # final_gate = ori_gate + shift_gate

            # v3.1 inter gate output
            # v3.2 add alpha
            # alpha = F.sigmoid(self.scale(hidden_state_shift - hidden_state)) * 2
            # shift_gate = self.W(self.R(hidden_state_shift - hidden_state)) * alpha

            # # 3.3 concat
            # shift_gate = self.W(self.R(torch.concat([hidden_state_shift, hidden_state], dim=-1)))
            # ori_gate = self.gate_proj(hidden_state)
...
#     def __init__(self, num_key_value_heads):
#         super().__init__()
#         self.num_key_value_heads = num_key_value_heads

#         # # 初始化参数
#         # self.alpha1 = nn.Parameter(torch.ones(num_key_value_heads))
...
#         self.shift_k_weight = nn.Linear(256, 2, bias=False)
#         self.shift_v_weight = nn.Linear(256, 2, bias=False)

#         with torch.no_grad():
#             self.shift_k_weight.weight[1].zero_()
#             self.shift_v_weight.weight[1].zero_()
...
#         # self.alpha1.data = torch.rand(self.num_key_value_heads, device=self.alpha1.device)
#         # self.alpha2.data = torch.ones(self.num_key_value_heads, device=self.alpha1.device) - self.alpha1
#         # self.beta1.data = torch.rand(self.num_key_value_heads, device=self.alpha1.device)
#         # self.beta2.data = torch.ones(self.num_key_value_heads, device=self.alpha1.device) - self.beta1

#         # self.s_score.reset_parameters()
#         # self.shift_v_score.reset_parameters()
...
#     def forward(self, key_states, value_states, last_key_states, last_v_states):
#         self._initialize_params()

#         bsz, num_head, seq_len, dim = key_states.shape
#         if seq_len > 1:
#             # assert (last_key_states is None) and (last_v_states is None), "last_key_states and last_v_states must be None for seq_len > 1"
#             # Get shifted versions
#             k_shifted = self._shift_gate(key_states)
#             v_shifted = self._shift_gate(value_states)
#         else:
#             # assert (last_key_states is not None) and (last_v_states is not None), "last_key_states and last_v_states must be provided for seq_len=1"

#             k_shifted = last_key_states
#             v_shifted = last_v_states

#         score1 = F.softmax(self.shift_k_weight(torch.cat([key_states, k_shifted], dim=-1)), dim=-1).bfloat16()
#         score2 = F.softmax(self.shift_v_weight(torch.cat([value_states, v_shifted], dim=-1)), dim=-1).bfloat16()

#         # Combine original and shifted versions with learned parameters
#         key_states = score1[:, :, :, 0].unsqueeze(3) * key_states + score1[:, :, :, 1].unsqueeze(3) * k_shifted
#         value_states = score2[:, :, :, 0].unsqueeze(3) * value_states + score2[:, :, :, 1].unsqueeze(3) * v_shifted

#         # print('k: ', torch.mean(score1, dim=(0,1,2)))
#         # print('v: ', torch.mean(score2, dim=(0,1,2)))
...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        # self.shift_gate = KVShiftingAttention(self.num_key_value_heads)
...
class Qwen2FlashAttention2(Qwen2Attention):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.last_k_states = self.last_v_states = None
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.mlp = Qwen2MLP_shift(config, layer_idx)
