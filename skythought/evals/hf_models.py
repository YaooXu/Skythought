...
from .util.hf_model_utils import (
    Instance,
    Collator,
    clear_torch_cache,
    configure_pad_token,
...
class HFLM:
        ...
        **kwargs,
        ...
        if not isinstance(pretrained, str):
            ...
            if not (parallelize or accelerator.num_processes > 1):
                ...
                device_list = set(["cuda", "cpu"] + [f"cuda:{i}" for i in range(gpus)])
    ...
    def _get_accelerate_args(
        ...
        if parallelize:  # Model parallelism will be used
            ...
            print(device_map)
            eval_logger.info(f"Model parallel was set to False, max memory was not set, and device map was set to {device_map}")
        else:
            args["max_memory"] = None
        ...
        **If not calling HFLM.__init__() or HFLM._get_backend() within a subclass of HFLM,
        ...
        if self.AUTO_MODEL_CLASS is None:
            if self.backend == "causal":
                if 'shift' in self.pretrained:
                    from llamafactory.custom_models.modeling_qwen2_shift_gate import Qwen2ForCausalLM
                    self.AUTO_MODEL_CLASS = Qwen2ForCausalLM
                    eval_logger.info(f"Using shift_gate model!")
                else:
    ...
    def generate_until(self, prompts: List[str], disable_tqdm: bool = False, **gen_kwargs) -> List[str]:
        res = []
        ...
        batch_size = (
            self.batch_size if self.batch_size != "auto" else adaptive_batch_size if adaptive_batch_size is not None else 0
        )
        ...
        for chunk in chunks:
            ...
            context_encs, attn_masks = self.tok_batch_encode(
                contexts,
                left_truncate_len=max_ctx_len,
                truncation=self.truncation,
            )
            context_encs = context_encs.to(self.device)
            attn_masks = attn_masks.to(self.device)

            if "max_length" not in kwargs:
                kwargs["max_length"] = context_encs.shape[1] + max_gen_toks

            # perform batched generation
            cont = self._model_generate(
                context=context_encs,
            ...
            for cont_toks, context_enc in zip(cont_toks_list, context_encs):
                # discard context + left-padding toks if using causal decoder-only LM
                if self.backend == "causal":
                    cont_toks = cont_toks[context_encs.shape[1] :]
                ...
                res.append({"response_texts": s, "response_tokens": cont_toks, "prompt_token_ids": context_enc})
